...
```
┌──────────────────────────────────────────────────────────────┐
│  可用插件列表  3 个                          [导出全部] [导入] [刷新] │
├──────────────────────────────────────────────────────────────┤
│  ☑ browser-use  v1.0  ✓已安装  │ 2 skills · 1 mcp │ [导出][编辑] │
│  ☐ computer-use v1.0            │ 1 skills · 0 mcp │ [导出][编辑] │
│  ☑ core         v1.0  ✓已安装  │ 5 skills · 3 mcp │ [导出][编辑] │
└──────────────────────────────────────────────────────────────┘
```

| 操作 | 说明 |
|------|------|
| **勾选安装/取消** | 勾选后自动安装插件（MCP 合并到 mcp.yaml、Skills 复制到 config/skills/），取消勾选则卸载 |
| **导入** | 导入外部 `.plugin.yaml` 文件 |
| **导出 / 导出全部** | 导出插件配置为 `.plugin.yaml` 文件 |
| **编辑** | 在插件构建页打开该插件进行编辑 |
| **刷新** | 重新扫描 `template/plugins/` 目录 |

> 插件安装后，其 MCP 服务和 Skills 会自动合并到全局配置，同步到各 IDE 时一并生效。

#### ⑧ 插件构建

![插件构建](docs/screenshots/plugin-build.png)

从本地仓库或 IDE 扫描结果，选择 LLM / MCP / Skill 组合，打包生成 `.plugin.yaml` 插件文件。
...
```
┌────────────┬────────────┬────────────┬──────────────┐
│ 本地 LLM    │ 本地 MCP    │ 本地 Skill  │ 插件定义      │
│ (5)         │ (22)        │ (6)         │              │
│ ┌──────────┐│┌──────────┐│┌──────────┐│ name: _____  │
│ │☑ openai  │││☑ 12306   │││☑ openspec││ version: 1.0 │
│ │  openai  │││  BOSS直聘 │││  explore ││ desc: ______ │
│ │☑ deepseek│││☑ Redis   │││☑ openrou-││ author: ____ │
│ │  anthropic││  amap     │││  ter-icu ││              │
│ └──────────┘│└──────────┘│└──────────┘│ [预览][保存]  │
│             │  [过滤...]  │  [分类▼]   │ [生成配置]    │
└────────────┴────────────┴────────────┴──────────────┘
```
...
| 操作 | 说明 |
|------|------|
| **勾选 LLM** | 从 `llm.yaml` 中选择要打包的 Provider/Protocol |
| **勾选 MCP** | 从 `mcp.yaml` 中选择要打包的 MCP Server（支持过滤搜索） |
| **勾选 Skill** | 从 `template/skills/` 中选择要打包的技能（支持分类筛选 + 搜索） |
| **填写插件信息** | name（必填）、version、description、author、install_script |
| **预览** | 预览生成的 `.plugin.yaml` JSON 内容 |
| **保存** | 保存到 `template/plugins/<name>.plugin.yaml` |
| **生成配置** | 保存并立即安装（合并 MCP/Skills 到全局配置） |
| **加载已有** | 从下拉菜单加载已有插件进行编辑 |

**模式二：IDE 导入向导**（5 步流程）

```
┌───────────────────────────────────────────────────────┐
│ ① 扫描 IDE → ② 选 LLM → ③ 选 MCP → ④ 选 Skill → ⑤ 生成 │
└───────────────────────────────────────────────────────┘
```

| 步骤 | 说明 |
|------|------|
| **① 扫描 IDE** | 一键扫描本地所有已安装 IDE 的 LLM/MCP/Skill 配置 |
| **② 选 LLM** | 勾选从 IDE 扫描到的 LLM Provider（支持「全选导入」） |
| **③ 选 MCP** | 勾选从 IDE 扫描到的 MCP Server + 本地仓库 MCP |
| **④ 选 Skill** | 勾选技能（支持分类筛选 + 搜索） |
| **⑤ 生成** | 填写插件名/版本/描述，预览/保存/安装 |

> IDE 导入向导适合「从现有 IDE 配置反向生成可分发插件」的场景。
